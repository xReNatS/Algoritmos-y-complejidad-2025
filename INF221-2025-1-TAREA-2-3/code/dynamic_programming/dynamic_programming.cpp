...
int main() {
    const string input_dir = "data/dynamic_programming_input";
    const string output_dir = "data/dynamic_programming_output";
    ...
    for (const auto& entry : archivos) {
        ...
        //k > 1000
        if (false) {
            duracion_total = 60000;
            memoria_usada = 18014398509481944ULL;
            cout << "Archivo con k > 1000. No se ejecuta el análisis, se fijan valores." << endl;
        } else {
            ...
            for (int caso = 1; caso <= k; ++caso) {
                ...
                auto pares = resolver_diferencias_dp(sec_s, sec_t);
